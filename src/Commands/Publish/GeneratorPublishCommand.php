...
class GeneratorPublishCommand extends PublishBaseCommand
{
     ...
     * @var string
     */
    protected $name = 'hublinkaz:publish';
    ...
    {
        $apiVersion = config('hublinkaz.laravel_generator.api_version', 'v1');
        $apiPrefix = config('hublinkaz.laravel_generator.api_prefix', 'api');
    ...
    {
        $testsPath = config('hublinkaz.laravel_generator.path.tests', base_path('tests/'));
        $testsNameSpace = config('hublinkaz.laravel_generator.namespace.tests', 'Tests');
        $createdAtField = config('hublinkaz.laravel_generator.timestamps.created_at', 'created_at');
        $updatedAtField = config('hublinkaz.laravel_generator.timestamps.updated_at', 'updated_at');
        ...
        $testAPIsPath = config('hublinkaz.laravel_generator.path.api_test', base_path('tests/APIs/'));
        if (!file_exists($testAPIsPath)) {
            FileUtil::createDirectoryIfNotExist($testAPIsPath);
            $this->info('APIs Tests directory created');
        }

        $testRepositoriesPath = config('hublinkaz.laravel_generator.path.repository_test', base_path('tests/Repositories/'));
