...
namespace Hublinkaz\Generator\Common;
...
class GeneratorConfig
{
    ...
    public function loadNamespaces(CommandData &$commandData)
    {
        ...
        $this->nsRepository = config('hublinkaz.laravel_generator.namespace.repository', 'App\Repositories').$prefix;
        $this->nsModel = config('hublinkaz.laravel_generator.namespace.model', 'App\Models').$prefix;
        if (config('hublinkaz.laravel_generator.ignore_model_prefix', false)) {
            $this->nsModel = config('hublinkaz.laravel_generator.namespace.model', 'App\Models');
        }
        $this->nsSeeder = config('hublinkaz.laravel_generator.namespace.seeder', 'Database\Seeders').$prefix;
        $this->nsFactory = config('hublinkaz.laravel_generator.namespace.factory', 'Database\Factories').$prefix;
        $this->nsDataTables = config('hublinkaz.laravel_generator.namespace.datatables', 'App\DataTables').$prefix;
        $this->nsModelExtend = config(
            'hublinkaz.laravel_generator.model_extend_class',
            'Illuminate\Database\Eloquent\Model'
        );

        $this->nsApiController = config(
            'hublinkaz.laravel_generator.namespace.api_controller',
            'App\Http\Controllers\API'
        ).$prefix;
        $this->nsApiResource = config(
            'hublinkaz.laravel_generator.namespace.api_resource',
            'App\Http\Resources'
        ).$prefix;
        $this->nsApiRequest = config('hublinkaz.laravel_generator.namespace.api_request', 'App\Http\Requests\API').$prefix;

        $this->nsRequest = config('hublinkaz.laravel_generator.namespace.request', 'App\Http\Requests').$prefix;
        $this->nsRequestBase = config('hublinkaz.laravel_generator.namespace.request', 'App\Http\Requests');
        $this->nsBaseController = config('hublinkaz.laravel_generator.namespace.controller', 'App\Http\Controllers');
        $this->nsController = config('hublinkaz.laravel_generator.namespace.controller', 'App\Http\Controllers').$prefix;

        $this->nsApiTests = config('hublinkaz.laravel_generator.namespace.api_test', 'Tests\APIs');
        $this->nsRepositoryTests = config('hublinkaz.laravel_generator.namespace.repository_test', 'Tests\Repositories');
        $this->nsTests = config('hublinkaz.laravel_generator.namespace.tests', 'Tests');
    }
    ...
    public function loadPaths()
    {
        ...
        $this->pathRepository = config(
            'hublinkaz.laravel_generator.path.repository',
            app_path('Repositories/')
        ).$prefix;

        $this->pathModel = config('hublinkaz.laravel_generator.path.model', app_path('Models/')).$prefix;
        if (config('hublinkaz.laravel_generator.ignore_model_prefix', false)) {
            $this->pathModel = config('hublinkaz.laravel_generator.path.model', app_path('Models/'));
        }

        $this->pathDataTables = config('hublinkaz.laravel_generator.path.datatables', app_path('DataTables/')).$prefix;

        $this->pathApiController = config(
            'hublinkaz.laravel_generator.path.api_controller',
            app_path('Http/Controllers/API/')
        ).$prefix;

        $this->pathApiResource = config(
            'hublinkaz.laravel_generator.path.api_resource',
            app_path('Http/Resources/')
        ).$prefix;

        $this->pathApiRequest = config(
            'hublinkaz.laravel_generator.path.api_request',
            app_path('Http/Requests/API/')
        ).$prefix;

        $this->pathApiRoutes = config('hublinkaz.laravel_generator.path.api_routes', base_path('routes/api.php'));

        $this->pathApiTests = config('hublinkaz.laravel_generator.path.api_test', base_path('tests/APIs/'));

        $this->pathController = config(
            'hublinkaz.laravel_generator.path.controller',
            app_path('Http/Controllers/')
        ).$prefix;

        $this->pathRequest = config('hublinkaz.laravel_generator.path.request', app_path('Http/Requests/')).$prefix;

        $this->pathRoutes = config('hublinkaz.laravel_generator.path.routes', base_path('routes/web.php'));
        $this->pathFactory = config('hublinkaz.laravel_generator.path.factory', database_path('factories/'));

        $this->pathViews = config(
            'hublinkaz.laravel_generator.path.views',
            resource_path('views/')
        ).$viewPrefix.$this->mSnakePlural.'/';

        $this->pathAssets = config(
            'hublinkaz.laravel_generator.path.assets',
            resource_path('assets/')
        );

        $this->pathSeeder = config('hublinkaz.laravel_generator.path.seeder', database_path('seeders/'));
        $this->pathDatabaseSeeder = config('hublinkaz.laravel_generator.path.database_seeder', database_path('seeders/DatabaseSeeder.php'));
        $this->pathViewProvider = config(
            'hublinkaz.laravel_generator.path.view_provider',
            app_path('Providers/ViewServiceProvider.php')
        );

        $this->modelJsPath = config(
            'hublinkaz.laravel_generator.path.modelsJs',
    ...
    public function loadDynamicVariables(CommandData &$commandData)
    {
        ...
        $commandData->addDynamicVariable(
            '$API_PREFIX$',
            config('hublinkaz.laravel_generator.api_prefix', 'api')
        );

        $commandData->addDynamicVariable(
            '$API_VERSION$',
            config('hublinkaz.laravel_generator.api_version', 'v1')
        );
    ...
    public function prepareOptions(CommandData &$commandData)
    {
        ...
        if (empty($this->options['save'])) {
            $this->options['save'] = config('hublinkaz.laravel_generator.options.save_schema_file', true);
        }

        if (empty($this->options['localized'])) {
            $this->options['localized'] = config('hublinkaz.laravel_generator.options.localized', false);
        }
        ...
        $this->options['softDelete'] = config('hublinkaz.laravel_generator.options.softDelete', false);
        $this->options['repositoryPattern'] = config('hublinkaz.laravel_generator.options.repository_pattern', true);
        $this->options['resources'] = config('hublinkaz.laravel_generator.options.resources', true);
    ...
    {
        $this->prefixes['route'] = explode('/', config('hublinkaz.laravel_generator.prefixes.route', ''));
        $this->prefixes['path'] = explode('/', config('hublinkaz.laravel_generator.prefixes.path', ''));
        $this->prefixes['view'] = explode('.', config('hublinkaz.laravel_generator.prefixes.view', ''));
        $this->prefixes['public'] = explode('/', config('hublinkaz.laravel_generator.prefixes.public', ''));
    ...
    {
        $this->addOns['swagger'] = config('hublinkaz.laravel_generator.add_on.swagger', false);
        $this->addOns['tests'] = config('hublinkaz.laravel_generator.add_on.tests', false);
        $this->addOns['datatables'] = config('hublinkaz.laravel_generator.add_on.datatables', false);
        $this->addOns['menu.enabled'] = config('hublinkaz.laravel_generator.add_on.menu.enabled', false);
        $this->addOns['menu.menu_file'] = config('hublinkaz.laravel_generator.add_on.menu.menu_file', 'layouts.menu');
    }
}
